...
#   print(local_file)
    ...
    idNo_text = jsonXML["teiHeader"]["fileDesc"][
        "publicationStmt"]["idno"][0]["#text"]
    idNo_type = jsonXML["teiHeader"]["fileDesc"][
        "publicationStmt"]["idno"][0]["@type"]
    title = jsonXML["teiHeader"]["fileDesc"]["titleStmt"]["title"]
    classCode = jsonXML["teiHeader"]["profileDesc"][
        "textClass"]["classCode"]["#text"]
    catRef = jsonXML["teiHeader"]["profileDesc"][
        "textClass"]["catRef"]["@targets"]
...
def add_data_to_index(es, index_name, document_type, data):
    ...
    result = es.index(index=index_name, doc_type=document_type,
                      body=data, request_timeout=30)
    return result
...
#      print(myList[0])
        if len(myList) < 1:
            ...
            sunitDict['origText'] = myList[1]
            sunitDict['rawText'] = myList[2]
            sunitDict['rawWordForms'] = myList[3]
            sunitDict['lemma'] = myList[4]
            sunitDict['pos'] = myList[5]
            sunitDict['mixed'] = myList[6]
#         pprint(sunitDict)
            sunitJSON = json.dumps(sunitDict)
#         pprint(sunitJSON)
            indexed = add_data_to_index(es, esIndex, esType, sunitJSON)
...
for dirpath, dirs, files in os.walk(datadir):
    for file in files:
        if re.search(txt_files, file) and re.search(segmented, dirpath):
            #         print(dirpath)
            print(file)
            return_value = split_and_index_text(
                dirpath, file, es, "bnces", "bncall")
