...
query = query.strip()
if re.search(" ", query):
   result = complex_query(es, "ice", "iceraw", query, max_no_hits, filters)
else:
   result = simple_query(es, "ice", "iceraw", query, max_no_hits, filters)
...
for row in sunit["hits"]["hits"]:
    print (row["_source"]["areaCode"])
    print (row["_source"]["region"])
    print (row["_source"]["textType"])
